...
* **`classes/privacy/provider.php`** — the gap flagged in an earlier pass — is now at
  **100% lines and methods (7/7)**, including three non-module-context guards, an
  orphaned-course-module guard, and an empty-user-list guard that a plain happy-path test would
  never reach.
...
* **`db/upgrade.php`** (0/55 lines) is the historical schema-migration script. Per project
  ...
  coverage above sits below the near-100% each individual class reaches.
